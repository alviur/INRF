...
class INRF(nn.Module):
    ...
    def __init__(self, dimG=1, dimW=3, numChanIn=3, numChanOut=3, sigma= 'relu', paramSigma= 0.6, lambdaV = 2.0, stride=1):
        ...
        #self.w = nn.Parameter(torch.empty(dimW*dimW, numChanOut, numChanIn, 1, 1).normal_(mean=0, std=paramSigma))
        ...
        #self.pandQ = nn.Parameter(torch.randn(2)+ 0.5)
    ...
    def forward(self, x):
        ...
        for i in range(self.dimW * self.dimW):
            ...
            #sub = 0.8*convw + self.lamda*self.activation(x, convw, self.sigma)# Experimentos locos

            sub = convw +  self.activation(x, convw, self.sigma)

            if i == 0:

                #w = torch.nn.functional.conv2d(sub, self.w)
                #w = self.w(sub)
        ...
        #outLayer = torch.nn.functional.conv2d(catTensor, self.w, stride=[1, 1, 1], padding=[0,0,0])
    ...
    def activation(self, x, convx, option):
        ...
        if(option==1):
            ...
            #newdiff = torch.where(diff > 0, (diff) ** (self.pandQ[0]), -torch.abs(diff) ** (self.pandQ[1]))
